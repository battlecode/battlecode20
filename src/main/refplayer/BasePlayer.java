...
import refplayer.message.EvilSender;
...
import refplayer.navigation.Navigation;
import refplayer.navigation.QueuedAction;
...
public abstract class BasePlayer {
	...
	public static final int ENEMY_PURSUE_TIME = 30;
	...
	static public final int AT_WAR_TIME = 15;
	...
	public BasePlayer(RobotController RC) {
		...
		evilSender = new EvilSender(this);
	...
	static private boolean release = true;
	...
	public RobotInfo nearestOneOf(FastList l, RobotInfo info) {
		int d, dmin;
		if(info!=null)
			dmin=myLoc.distanceSquaredTo(info.location);
		else
			dmin=99999;
		int i;
		RobotInfo best = info;
	...
	public RobotInfo nearestEnemyGround() {
		RobotInfo info = nearestOneOf(enemyChainers);
		info = nearestOneOf(enemySoldiers,info);
		info = nearestOneOf(enemyTurrets,info);
		info = nearestOneOf(enemyAuras,info);
		info = nearestOneOf(enemyComms,info);
		info = nearestOneOf(enemyTeleporters,info);
		info = nearestOneOf(enemyWouts,info);
		return info;
	}
