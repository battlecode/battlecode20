...
import refplayer.goals.ArchonExploreGoal;
import refplayer.goals.ArchonFindEnemyGoal;
import refplayer.goals.BroadcastGoInThisDirectionGoal;
import refplayer.goals.FindEnemyGoal;
import refplayer.goals.Goal;
import refplayer.goals.BroadcastEnemyUnitLocationsGoal;
import refplayer.goals.StayTogetherGoal;
...
public class ArchonPlayer extends BasePlayer {
	...
	public static final double SUPPORT_RATIO = .1;
	...
	public void setGoals() {
		...
		broadcastGoals = new Goal [] {
			new BroadcastGoInThisDirectionGoal(this, exploreGoal),
	...
	public void transferFlux() {
		...
		try {
			...
			for(i=alliedArchons.size-1;i>=0;i--) {
				info = infos[i];
				if(info.location.isAdjacentTo(myLoc)&&alliedArchonRobots[i].getID()<myID) {
	...
	public RobotType chooseTypeToSpawn() {
		...
		if(alliedSoldiers.size>alliedWouts.size)
