package teleportingplayer;

import teleportingplayer.goals.ArchonDistributeFluxGoal;
import teleportingplayer.goals.BroadcastTeleportGoal;
import teleportingplayer.goals.Goal;
import teleportingplayer.goals.MakeChainGoal;
import teleportingplayer.goals.RunAwayGoal;
import teleportingplayer.goals.WanderGoal;
...
public class ArchonPlayer extends BasePlayer {
	...
	public static final double SUPPORT_RATIO = .2;
	...
	public void run() {
		setGoals();
		while(true) {
			//debug_setIndicatorStringObject(1,myRC.getLocation());
			try {
	...
	public void makeArmy() {
		...
		if(typeToSpawn.isBuilding()) {
			if(myRC.getFlux()<typeToSpawn.spawnFluxCost())
				return;
		}
		else {
			if(myRC.getEnergonLevel()<typeToSpawn.spawnCost()+MIN_ENERGON)
				return;
		}
		if(typeToSpawn==RobotType.WOUT&&!canSupportAnother(RobotType.WOUT))
			return;
		try {
