...
public class ArchonDistributeFluxGoal extends Goal {
	...
	static public final double flux_per_teleporter=TELEPORT_FLUX_COST*2;
		...
		try {
			...
			if(targetInfo!=null) {
				if(myLoc.distanceSquaredTo(targetInfo.location)<=2) {
					double transferAmount=Math.min(myFlux,RobotType.ARCHON.maxFlux()-targetInfo.flux);
					myRC.transferFlux(myFlux,targetInfo.location,RobotLevel.IN_AIR);	
				}
				else if(targetInfo.flux<RobotType.ARCHON.maxFlux()-min_flux) {
