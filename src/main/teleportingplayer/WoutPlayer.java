...
import teleportingplayer.goals.SeekEnergonGoal;
import teleportingplayer.goals.TransportEnergonGoal;
import teleportingplayer.goals.WoutDistributeFluxGoal;
import teleportingplayer.goals.WoutSpreadOutGoal;
import teleportingplayer.goals.WoutSuicideGoal;
		...
		try {
			...
			for(i=alliedArchons.size-1;i>=0;i--) {
				...
				if(infos[i].location.distanceSquaredTo(myLoc)<=2) {
					double transferAmount = Math.min(myFlux,RobotType.ARCHON.maxFlux()-infos[i].flux);
					myRC.transferFlux(transferAmount,infos[i].location,RobotLevel.IN_AIR);
					return;
				}
			}
		} catch(Exception e) {
