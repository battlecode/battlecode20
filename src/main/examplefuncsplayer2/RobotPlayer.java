			...
			if (rc.getType() == RobotType.SOLDIER) {
				try {
					if (rc.isActive()) {
						...
						} else if (action < 2 && rc.getLocation().distanceSquaredTo(rc.senseHQLocation()) > 2) {
			...
			if (rc.getType() == RobotType.NOISETOWER) {
				try{
					if (rc.isActive()) {
						...
						if (action == 0) {
							rc.attackSquare(rc.getLocation().add(Direction.NORTH));
						} else {
							rc.attackSquareLight(rc.getLocation().add(Direction.SOUTH));
						}
					}
				} catch (Exception e) {
