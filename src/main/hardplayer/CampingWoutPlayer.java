...
import hardplayer.goals.FillUpSeekEnergonGoal;
...
import hardplayer.goals.ShareFluxGoal;
import hardplayer.goals.WoutSpreadOutGoal;
...
public class CampingWoutPlayer extends WoutPlayer {
	
	public CampingWoutPlayer(RobotController RC) {
		super(RC);
	}
	...
	public void setGoals() {
		...
		tmh = new TeleporterMessageHandler(this);

		movementGoals = new Goal [] {
			new WoutSpreadOutGoal(this,tmh),
			new ShareFluxGoal(this),
			new WoutAttackBuildingGoal(this),
			new FillUpSeekEnergonGoal(this)
		};
	...
	public void spawn() {
		if(myRC.getFlux()>=4500&&myRC.canMove(myRC.getDirection())) {
			try {
	...
	public void transferFlux() {
		...
		try {
			RobotInfo info;
			int i;
			RobotInfo [] infos = alliedTeleporterInfos;
			for(i=alliedTeleporters.size-1;i>=0;i--) {
				info = infos[i];
				if(info.location.isAdjacentTo(myLoc)) {
					transferAmount = Math.min(myFlux,ENERGON_TO_FLUX_CONVERSION*(MAX_RESERVE-info.eventualEnergon));
					if(transferAmount<=0) continue;
			...
			*/
			infos = alliedWoutInfos;
			for(i=alliedWouts.size-1;i>=0;i--) {
				info = infos[i];
				if(info.id<myID&&info.location.distanceSquaredTo(myLoc)<=2) {
