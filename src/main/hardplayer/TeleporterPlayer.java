...
import hardplayer.message.BuildingMessageSender;
...
public class TeleporterPlayer extends BasePlayer implements MessageHandler {
	...
	static public final int SENSE_EVERY = 20;
	...
	public TeleporterPlayer(RobotController RC) {
		...
		mySender = new BuildingMessageSender(this);
	}
		...
		while(true) {
			try {
				if(myRC.getFlux()>=TELEPORT_FLUX_COST) {
					myRC.setIndicatorString(0,"enough");
					sortMessages();
					teleport:
					...
					myRC.setIndicatorString(0,"not enough");
					mySender.updateRoundNum();
					myRC.getAllMessages();
					teleporterMessages.clear();
				}
				if(Clock.getRoundNum()>=nextSense) {
					senseNearbyRobots();
					if(alliedWouts.size>0) {
						...
						nextSense=Clock.getRoundNum()+SENSE_EVERY;
					}
				}
			} catch(Exception e) {
