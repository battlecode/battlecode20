...
public class NewChainerFindEnemyGoal extends FindEnemyGoal {

	public NewChainerFindEnemyGoal(BasePlayer bp) {
		super(bp);
	}
	...
	public void tryToAccomplish() {
		...
		int i;
		int dist=player.myLoc.add(enemyLoc.directionTo(player.myLoc)).distanceSquaredTo(enemyLoc);
		...
		*/
		...
		for(i=archons.length-1;i>=0;i--) {
			...
			if(archons[i].distanceSquaredTo(enemyLoc)<dist&&archons[i].distanceSquaredTo(player.myLoc)<=16){
				player.myNav.moveToForward(enemyLoc);
				return;
			}
		}
